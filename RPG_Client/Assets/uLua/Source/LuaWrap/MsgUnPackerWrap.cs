...
public class MsgUnPackerWrap
{
	public static void Register(IntPtr L)
	{
		...
		{
			new LuaField("RecvHandler", get_RecvHandler, set_RecvHandler),
	...
	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int get_RecvHandler(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			{
				LuaDLL.luaL_error(L, "unknown member name RecvHandler");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index RecvHandler on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.RecvHandler);
		return 1;
	}
	...
	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int set_RecvHandler(IntPtr L)
	{
		object o = LuaScriptMgr.GetLuaObject(L, 1);
		MsgUnPacker obj = (MsgUnPacker)o;
		...
		{
			obj.RecvHandler = (MsgHandler)LuaScriptMgr.GetNetObject(L, 3, typeof(MsgHandler));
		}
		else
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			obj.RecvHandler = (param0) =>
			{
